...
import { Zap, Flame, GraduationCap, Settings, Award, CheckCircle, ArrowRight, BookOpen, MessageSquare, Briefcase, FileText, CheckSquare, Star, Volume2 } from 'lucide-react';
import { useAppContext } from '../context/AppContext';

const boosterQuestions = [
    {
    ...
    },
    {
        ...
        correct: 2,
        funFact: 'This word is commonly misspelled in competitive exams! Remember: double c, double m.'
    },
    {
        question: 'What is the antonym (opposite) of "Benevolent"?',
        options: ['A) Generous', 'B) Malevolent', 'C) Kind', 'D) Calm'],
        correct: 1,
        funFact: '"Bene" means good (benefit, beneficial). "Male" means bad (malice, malicious).'
    },
    {
    ...
    },
    {
        ...
        correct: 1,
        funFact: 'This phrase is commonly used in MNC workplaces to describe new hires who adapt quickly.'
    },
    {
    ...
    },
    {
        ...
        correct: 1,
        funFact: '"Articulate" and "Eloquent" both describe someone who speaks clearly and effectively.'
    }
];

const getTodayBooster = () => {
...
const Dashboard = () => {
    ...
    const [todaysBooster] = useState(getTodayBooster);

    // Persisted daily checklist
    const [checklist, setChecklist] = useState(() => {
        const today = new Date().toDateString();
        const saved = localStorage.getItem('speakup_checklist');
        if (saved) {
            try {
    ...
    useEffect(() => {
        const today = new Date().toDateString();
        localStorage.setItem('speakup_checklist', JSON.stringify({ date: today, items: checklist }));
    }, [checklist]);
    ...
    const handleBoosterClick = (isCorrect) => {
        if (isCorrect) {
            ...
            showToast(`🎉 Correct! +30 XP claimed. ${todaysBooster.funFact ? '💡 ' + todaysBooster.funFact : ''}`);
        } else {
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                    ...
                    <div className="card">
                        <h3 style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px' }}>
                            <GraduationCap color="var(--primary-color)" size={22} /> SpeakUp Expert Career Roadmap
                        </h3>
                    ...
                    <div className="card">
                        ...
                        <div style={{ background: 'rgba(255,255,255,0.01)', border: '1px dashed var(--border-color)', borderRadius: '8px', padding: '20px' }}>
                            {hasClaimed ? (
                                ...
                                <div>
                                    <h4 style={{ fontSize: '15px', marginBottom: '12px' }}>Quick Challenge: {todaysBooster.question}</h4>
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                                        {todaysBooster.options.map((opt, i) => (
                                            <button
                                                key={i}
                                                className="btn-secondary"
                                                onClick={() => handleBoosterClick(i === todaysBooster.correct)}
                                                style={{ textAlign: 'left', padding: '12px', fontSize: '13px' }}
                                            >
                                                {opt}
                                            </button>
                                        ))}
                                    </div>
                                    {todaysBooster.funFact && (
                                        <div style={{ marginTop: '12px', padding: '10px', background: 'rgba(245,158,11,0.08)', borderRadius: '8px', fontSize: '12.5px', color: 'var(--text-muted)', borderLeft: '3px solid #f59e0b' }}>
                                            💡 {todaysBooster.funFact}
                                        </div>
                                    )}
                                </div>
