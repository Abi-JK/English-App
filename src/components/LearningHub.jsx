...
import { ChevronLeft, ChevronRight, BookOpen, Zap, MessageCircle, RotateCcw, CheckCircle2, Volume2, Gamepad2 } from 'lucide-react';
...
const VocabMatchGame = () => {
  ...
  const [wrongMatch, setWrongMatch] = useState(null); // { wordId, meaningId }
  const [gameWon, setGameWon] = useState(false);

  // Pool vocabulary words for the game
  const gamePool = [
  ...
  const initGame = () => {
    ...
    // Shuffle both sets together but independently
  ...
  const handleCardClick = (item) => {
    ...
    if (wrongMatch) return; // Prevent clicking during wrong-match red animation
  ...
  return (
    <div style={{ maxWidth: '600px', margin: '0 auto', textAlign: 'center' }}>
      <div style={{ marginBottom: '24px' }}>
        ...
        <p className="text-muted" style={{ fontSize: '13.5px' }}>Match the English vocabulary words on the left/right with their correct Tamil meaning cards. Active learning helps avoid distraction!</p>
      </div>
  ...
  return (
    <div>
      <div style={{ marginBottom: '28px' }}>
        ...
        <p className="text-muted" style={{ marginTop: '4px' }}>Build your vocabulary, master grammar, and learn idioms used in real life and exams.</p>
      </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '16px' }}>
                {rule.examples.map((ex, i) => (
                  <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                    <div style={{ padding: '10px 14px', borderRadius: '8px', background: '#ef444418', borderLeft: '3px solid #ef4444', fontSize: '14px' }}>
                      ❌ {ex.wrong}
                    </div>
                    <div style={{ padding: '10px 14px', borderRadius: '8px', background: '#10b98118', borderLeft: '3px solid #10b981', fontSize: '14px' }}>
                      ✅ {ex.right}
                    </div>
                  </div>
                ))}
        ...
        <div>
          <div style={{ marginBottom: '20px', padding: '16px', background: 'rgba(0,150,136,0.08)', borderRadius: '12px', borderLeft: '4px solid var(--primary-color)' }}>
            ...
            <p className="text-muted" style={{ fontSize: '14px', marginTop: '4px', marginBottom: 0 }}>
              Native English speakers and MNC professionals use idioms all the time. Knowing them makes you sound natural and confident during HR rounds and group discussions.
            </p>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '16px' }}>
            {idioms.map((item, i) => (
              <div key={i} className="card" style={{ borderLeft: '3px solid var(--primary-color)' }}>
                ...
                <p style={{ fontSize: '13px', color: 'var(--text-muted)', fontStyle: 'italic' }}>"{item.use}"</p>
              </div>
            ))}
          </div>
        </div>
